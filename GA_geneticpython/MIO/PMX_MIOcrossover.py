...
class PMXCrossover(Crossover):
    ...
    def set_n(self, n_op, n_machine):
        self.n_op = n_op
        self.n_machine = n_machine
    def custom_sort(self, num_list):
        return sorted(num_list, key=lambda x: (x % self.n_machine, x // self.n_machine))
    ...
    def cross(self, father: PermutationIndividual, mother: PermutationIndividual, random_state=None):
        ...
        # 0부터 99까지의 수 생성
        numbers = list(range(self.n_op))
        # 패턴대로 정렬
        sorted_numbers = self.custom_sort(numbers)
        ...
        chrom1 = deepcopy(child1.chromosome)
        ...
        p_1 = deepcopy(child1.chromosome.genes.tolist())
